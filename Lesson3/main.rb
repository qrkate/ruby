...
class Main
  ...
  private
  # вынесла, потому что методы должны выполняться только изнутри класса
  ...
  def assign_route
    ...
    train.add_route(route) if train.number == gets.chomp.to_i
  ...
  def show_trains_at_stations
    loop do
      ...
      when 2
        ...
        show_stations
        @stations[gets.chomp.to_i].trains.each { |train| puts "Поезд № #{train.number}" }
      end
    end
  end
  ...
  def show_stations
    @stations.each.with_index  { |station, n| puts "#{n} - #{station.name}" }
  end
  
  def new_route
    puts "Введите номерa начальной и конечной станции маршрута через пробел:"
    show_stations
    start, finish = gets.chomp.split.map { |index| @stations[index.to_i] }
